...
inherit multilib toolchain-funcs eutils user desktop

case ${EAPI:-0} in
	6|7) EXPORT_FUNCTIONS pkg_setup src_configure src_compile pkg_preinst pkg_postinst ;;
...
# @FUNCTION: games-r1_make_wrapper
...
games-r1_make_wrapper() { gameswrapper ${FUNCNAME/games-r1_} "$@"; }
...
prepgamesdirs() {
	...
	for dir in \
		...
		"${GAMES_SYSCONFDIR}" "${GAMES_STATEDIR}" "$(games-r1_get_libdir)" \
		"${GAMES_BINDIR}" "$@"
	do
		[[ ! -d ${D}/${dir} ]] && continue
		(
...
# @FUNCTION: games-r1_src_configure
# @DESCRIPTION:
# Runs egamesconf if there is a configure file.
# This function is exported as src_configure().
games-r1_src_configure() {
	[[ -x "${ECONF_SOURCE:-.}"/configure ]] && egamesconf
}

# @FUNCTION: games-r1_src_compile
# @DESCRIPTION:
# Runs emake(). This function is exported as src_compile().
games-r1_src_compile() {
	if [[ -f Makefile || -f GNUmakefile || -f makefile ]]; then
		emake "$@"
	fi
}

# @FUNCTION: games-r1_pkg_preinst
# @DESCRIPTION:
# Synchronizes GAMES_STATEDIR of the ebuild image with the live filesystem.
games-r1_pkg_preinst() {
	local f
...
# @FUNCTION: games-r1_pkg_postinst
# @DESCRIPTION:
...
games-r1_ut_unpack() {
	...
	if [[ -z ${ut_unpack} ]] ; then
		die "You must provide an argument to games-r1_ut_unpack"
	fi
	if [[ -f ${ut_unpack} ]] ; then
