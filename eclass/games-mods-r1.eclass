...
case ${GAME} in
	...
	quake3)
		...
		GAME_DIRS=( /usr/share/quake3 /opt/quake3 )
		GAME_NAME="Quake III"
		GAME_BIN="quake3"
		GAME_ICON="quake3"
		DED_PKGS=""
	...
	ut2004)
		...
		GAME_DIRS=( /opt/{ut2004,ut2004-ded} )
		GAME_NAME="UT2004"
		GAME_BIN="ut2004"
		GAME_ICON="ut2004"
	...
	*)
		...
		die "games-mods-r1.eclass: unsupported GAME"
		;;
esac
...
RDEPEND="dedicated? ( $(games-mods-r1_get_rdepend --ded) )
	opengl? ( $(games-mods-r1_get_rdepend) )
	!dedicated? ( !opengl? ( $(games-mods-r1_get_rdepend) ) )"

S=${WORKDIR}

INS_DIR=/usr/share/${GAME}

games-mods-r1_use_opengl() {
	[[ -z ${MOD_DIR} ]] && return 1
...
games-mods-r1_use_dedicated() {
	[[ -z ${MOD_DIR} ]] && return 1

	use dedicated && return 0 || return 1
}

games-mods-r1_dosyms() {
	# We are installing everything for these mods into ${INS_DIR},
	# /usr/share/${GAME} in most cases, and symlinking it
	# into /opt/${GAME} for each game.  This should
...
games-mods-r1_make_initd() {
	cat <<EOF > "${T}"/${MOD_DED_BIN}
#!/sbin/openrc-run
# Copyright 1999-2016 Gentoo Foundation
# Distributed under the terms of the GNU General Public License v2
# Generated by games-mods-r1.eclass
...
games-mods-r1_make_confd() {
	cat <<-EOF > "${T}"/${MOD_DED_BIN}
	# User and group the server should run as
	${MOD_DED_BIN//-/_}_user="${GAMES_USER_DED:-games}"
	${MOD_DED_BIN//-/_}_group="${GAMES_GROUP:-games}"

	# Directory to use for HOME
	${MOD_DED_BIN//-/_}_home=/var/games
...
games-mods-r1_src_install() {
	if games-mods-r1_use_opengl ; then
		if [[ -n ${MOD_ICON} ]] ; then
			# Install custom icon
			local ext=${MOD_ICON##*.}
			if [[ -f ${MOD_ICON} ]] ; then
		...
		games-r1_make_wrapper ${MOD_BIN} "${GAME_BIN} ${SELECT_MOD}${MOD_DIR}"
		...
		case ${GAME} in
			quake3)
				if has_version games-fps/quake3-bin ; then
					games-r1_make_wrapper ${GAME_BIN}-bin-${PN/${GAME}-} \
	...
	if games-mods-r1_use_dedicated ; then
		if [[ -f ${FILESDIR}/server.cfg ]] ; then
			insinto /etc/games/${GAME}/${MOD_DIR}
			doins "${FILESDIR}"/server.cfg || die "doins server.cfg failed"
			dosym /etc/games/${GAME}/${MOD_DIR}/server.cfg \
				/usr/share/${GAME}/${DED_CFG_DIR}/${MOD_DIR}/server.cfg \
				|| die "dosym server.cfg failed"
		fi
		games-r1_make_wrapper ${MOD_DED_BIN} \
			"\"/usr/bin/${DED_BIN}\" ${SELECT_MOD}${MOD_DIR} ${DED_OPTS}"
		games-mods-r1_make_initd
		games-mods-r1_make_confd
	fi
